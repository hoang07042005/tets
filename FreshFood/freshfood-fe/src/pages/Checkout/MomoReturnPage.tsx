...
export const MomoReturnPage = () => {
  ...
  const success = valid && code === '0';

  const orderCode = useMemo(() => {
    const x = orderId.trim();
    return x ? `#${x}` : '';
  }, [orderId]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        await apiService.markOrderPaymentFailed(id, uid, 'MOMO', code || null);
      } catch {
        // ignore
      }
    })();
  ...
  useEffect(() => {
    // Best effort: if we have a numeric orderId, fetch orderCode for nicer UI
    const id = Number(orderId);
    if (!success) return;
    if (!Number.isFinite(id) || id <= 0) return;
    let cancelled = false;
    (async () => {
      try {
        const o = await apiService.getOrder(id);
        if (!cancelled && o?.orderCode) {
          // No state; we keep the UI simple (orderId is enough).
        }
      } catch {
        // ignore
      }
    ...
    <div className="empty-state" style={{ padding: '5rem 2rem', textAlign: 'center' }}>
      {success ? (
        <>
          ...
          <p>Đơn hàng {orderCode} đã được thanh toán qua MoMo.</p>
        ...
        <>
          ...
          <p>{valid ? `Mã phản hồi: ${code}` : 'Chữ ký không hợp lệ hoặc dữ liệu trả về không đúng.'}</p>
